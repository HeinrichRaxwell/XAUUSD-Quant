...
export default function TradingViewTechSummary() {
  ...
  useEffect(() => {
    ...
    // Clean up previous widget DOM content before re-mounting
    ...
    try {
      ...
      // Fallback timer if script doesn't trigger onload
      const timer = setTimeout(() => setIsLoading(false), 2000);
      return () => clearTimeout(timer);
    } catch (err) {
      console.error('TradingView Technical Analysis widget mount error:', err);
      setHasError(true);
      setIsLoading(false);
    }
  }, []);
      ...
      <div style={{ flex: 1, position: 'relative', background: '#131722', borderRadius: '4px', overflow: 'hidden' }}>
        {isLoading && (
          <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#131722', color: '#4edea3', fontSize: '0.8rem', zIndex: 10 }}>
            ⚡ Loading Official TradingView Technical Gauge...
          </div>
        )}
